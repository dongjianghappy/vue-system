...
<v-drawer ref="drawer" v-model:show="isShow" :action="action" title="日志模板" :style="{width: '850'}" :hasfooter="false">
  <template v-slot:extra>
    <v-space>
      <Detail :data="{coding: coding.template}" :render="init" />
    </v-space>
  </template>
    ...
    <div class="col-md-4 p10" v-for="(item, index) in dataList.list" :key="index">
      <div class="p10" style="border: 1px solid #f0f0f0;">
        ...
        <div class="ptb15">{{item.name}}
          <Detail action="edit" :data="{id: item.id, coding: coding.template}" :render="init" />
