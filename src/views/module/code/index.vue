  ...
  <div class="module-content plr15">
    <table class="table-striped table-hover col-left-2">
      ...
      <tr v-for="(item, index) in dataList.list" :key="index">
        ...
        <td>
          <div class="pointer">
            <v-category title="选择分类" :name="item.parent ? item.parent : '选择分类'" :data="{item, coding}" :isMore="true" :isUpdate="true" type="text"></v-category>
          </div>
        </td>
        <td>{{item.visit}}</td>
        <td>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const coding: any = codings.module['code'];
