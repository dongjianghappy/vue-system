...
<v-button v-model:show="isShow" :disabled="auth">
  ...
  <template v-else>
    <i class="iconfont" :class="`icon-${action === 'add' && 'anonymous-iconfont'}`" />{{action === 'edit'? "编辑": "新增知识"}}
  </template>
</v-button>
<v-drawer ref="drawer" v-model:show="isShow" :action="action" :title="action === 'edit' ? '编辑内容' : '新增知识' " :data="{...data, coding: data.coding.art}" :param="detail" :render="render" :submit="submit">
