...
import { getCurrentInstance, ref, useStore } from "@/utils";
...
function run(prams: any) {
  ...
  store
    ...
    .then((res) => {
      const { type, bar, id, name, loop, status } = res.result;
      ...
      if (res.result.again) {
        const { serve, action, model, method, fid } = prams.data;
