  ...
  <div class="card h100" v-else>
    ...
    <div>
      <div class="col-md-2 limit relative" :class="`col-md-2`">
        <v-popover
          :content="`<div>排序: <span>${enumSort[item.sort]}</span></div>`"
          arrow="tb"
          offset="right"
          :move="-10"
          :keys="`static_${index}`"
        >
          <ul>
            <li @click="data.sort = i" v-for="(data, i) in sort" :key="i">
              {{ data }}
            </li>
          </ul>
        </v-popover>
      ...
      <div class="col-md-3 limit relative">
        <v-popover
          :content="`<span>范围: ${item.limitStart} - ${item.limitEnd}</span>`"
          arrow="tb"
          offset="right"
          :move="-100"
          :keys="`static_${index}`"
        >
          <div class="p10">
            <input
              ...
              type="text"
              class="ant-input"
              v-model="item.limitStart"
              style="width: 100px; flex: 1"
            /><span class="pl15 pr15"> 至 </span
            ><input
              maxlength="5"
              type="text"
              class="ant-input"
              v-model="item.limitEnd"
              style="width: 100px; flex: 1"
            />
          </div>
        </v-popover>
      ...
      <div class="col-md-1">
        <Progress
          :data="{ title: `正在更新${item.name}${title}`, data: data }"
        />
      </div>
    </div>
  </div>
