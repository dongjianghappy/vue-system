...
<div class="bg-white w200 silde-wrap" style="height: 100%;">
  ...
  <div class="aside" v-for="(item, index) in siderRouter" :key="index">
    <div class="aside-head" v-if="item.children" @click="handleShow(item)">
      ...
      <span v-if="collapse">{{item.name}}</span></div>
    <div class="aside-list" v-if="item.children" v-show="item.isshow">
      <ul v-if="collapse">
        <li class="pl50" v-for="(list, i) in item.children" :key="i" @click="handleclick(list)">
          <span v-if="collapse">{{list.name}}</span></li>
      </ul>
    </div>
    <div class="aside-list" v-else>
      <ul>

        <li @click="handleclick(item)"><i class="iconfont" :class="`icon-${item.icon}`"></i> <span v-if="collapse">{{item.name}}</span></li>
      </ul>
    </div>
  </div>
</div>
