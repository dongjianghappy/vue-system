          ...
          >
            <ul>
              <li
                ...
                @click="handleClicksss(item, i)"
                v-for="(item, i) in NAV_TYPE"
                :key="i"
              >
                {{ item.name }}
              </li>
          ...
          <tr :draggable="isMove" class="dragObj" :index="`0_${index}`">
            ...
            <td>
              ...
              <v-quick
                ...
                :style="{ width: '80%', ...jsonParse(item.style) }"
                width="80%"
                :auth="auth.checked('edit')"
              />
          ...
          >
            ...
            <td>
              ...
              <v-quick
                ...
                :style="{ width: '80%', ...jsonParse(data.style) }"
              />
            </td>
            <td>
              <v-quick
                :value="data.url"
                :data="{ id: data.id, field: 'url', coding }"
              />
            </td>
            <td>
...
import { NAV_TYPE } from "@/assets/enum";
...
const aaa: any = ref([]);
const auth: any = proxy.$auth.init("navigation/main");
    ...
    .then((res) => {
      ...
      setTimeout(() => {
        proxy.$drag.init((res: any) => {
          ...
          const list =
            res.findex !== null
              ? dataList.value[res.findex].list
              : dataList.value;
