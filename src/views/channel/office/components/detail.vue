...
<v-drawer ref="drawer" v-model:show="isShow" :action="action" :title="action === 'edit' ? '编辑内容' : '新增内容' " api="articleDetail" :data="{...data, coding: coding.art}" :param="detail" :render="render" :submit="submit">
  <template v-slot:content v-if="isShow">
    ...
    <div class="mb15" style="overflow: hidden;"><v-upload ref="upload" :data="{id: detail.id, cover: detail.cover,  coding: 'C20000'}" :dataList="detail.img || []" uploadtype="office" @imgList="image" :style="'width: 135px'" /></div>
  </template>
</v-drawer>
</template>
