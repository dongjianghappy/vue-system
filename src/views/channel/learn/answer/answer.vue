<template>
<v-button v-model:show="isShow" :disabled="auth">回答({{data.answer}})</v-button>
<v-drawer v-model:show="isShow" ref="form" :title="`留言回答`" :style="{width: 800}" :hasfooter="false" :confirm="true" :cancel="true" @submit="submit">
  <template v-slot:content v-if="isShow">
    <slot name="content">
      <table class="table-striped table-hover col-left-1">
        <tr class="th">
          <td class="col-md-2">用户</td>
          <td class="col-md-4">回答内容</td>
          <td class="col-md-3">回答时间</td>
...
import Praise from '../../../features/praise/list.vue'
  const props: any = defineProps({
    ...
    const coding: any = codings.talk.message_board.list;
