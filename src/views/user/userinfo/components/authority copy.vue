...
<Drawer v-model:show="isShow" :top="64" width="400" :auth="auth">
            ...
            <div>
              <ul class="align_left" style="padding-left: 30px;">
                <li class="li mb15" :class="{'extand': item.extand}" v-for="(list, i) in item.list" :key="i">
                  ...
                  <v-switch :data="{ item: list, field: 'authority', ...data }" :param="{uid: uid, field: list.value}" api="setUserAuthority" :msg="message" className="right" :auth="true" />
                </li>
              </ul>
            </div>
          </div>
