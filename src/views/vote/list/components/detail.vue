<template>
<v-button v-model:show="isShow">修改项目信息</v-button>
<Drawer ref="drawer" v-model:show="isShow" :action="action" :title="action === 'edit' ? '编辑投票' : '创建投票' " :data="data" :param="detail" :render="render">
  <template v-slot:content v-if="isShow">
    <ul class="form-wrap-box">
      ...
      <li class="li">
        ...
        <input v-model="detail.url" type="text" placeholder="请输入话题绑定" class="input-sm input-150" />
      </li>
      <li class="li">
        <span class="label">截止时间</span>
        <input v-model="detail.sort" type="text" placeholder="开始时间" class="input-sm input-150" />
        至
        <input v-model="detail.sort" type="text" placeholder="结束时间" class="input-sm input-150" />
      </li>
      <li class="li">
      ...
      <li class="li">
        ...
        <textarea placeholder="请输入投票说明" v-model="data.content" class="w-full"></textarea>
      </li>
    </ul>
